...
app.post('/stripe/webhook', express.raw({type: 'application/json'}), async (req, res) => {
  ...
  switch (event.type) {
    ...
    case 'charge.succeeded':
      ...
      console.log('💰 Payment succeeded (charge.succeeded):', charge.id);
      console.log('Charge metadata:', charge.metadata);
      
      // Si el charge tiene metadata, procesarlo
      if (charge.metadata && Object.keys(charge.metadata).length > 0) {
        ...
        try {
          ...
          if (!no_boletos || !cliente_id || !tourId) {
            console.log('⚠️ Metadata incompleta en charge:', charge.metadata);
            break;
          }
          ...
          console.log(`✅ Venta creada exitosamente desde charge.succeeded: ${id_reservacion}, viajeTourId: ${viajeTourId}`);
          
        } catch (error) {
          console.error('Error procesando pago en webhook (charge.succeeded):', error);
        }
      } else {
        console.log('🔍 Charge sin metadata relevante');
      }
      break;
