...
app.post('/verificar', auth, async (req, res) => {
	...
	try {
		let query = `SELECT id, nombres, apellidos, telefono, telefono_emergencia, correo, isClient, isAdmin, isSuperAdmin, isGuia, isInvestor, isPartner, isSpecialist, isOperator, status, created_at, empresa_id FROM usuario WHERE id='${req.user.id}'`;
		let client = await db.pool.query(query);
